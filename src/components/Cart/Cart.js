...
const Cart = () => {
    ...
    return ( 
        <div>
            <h1 className="tituloCa" >Carrito</h1>
            {(products.length > 0 && !processingOrder) && <h3>Total: ${getTotal()}</h3>}
            {(!processingOrder && products.length > 0) && <button onClick={() => clearCart()} className="Button btn btn-outline-success">Cancelar compra</button>}
            {(!processingOrder && products.length > 0) && <button onClick={() => confirmOrder()} className="Button btn btn-outline-success">Confirmar Compra</button>}
            {(!processingOrder && contact.phone !== '' && contact.address !== '' && contact.comment !== '') &&
                <div>
                    ...
                    <button onClick={() => setContact({ phone: '', address: '', comment: ''})}className="Button btn btn-outline-success" >Borrar datos de contacto</button>
                </div>    
            }
